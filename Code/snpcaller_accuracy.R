...
site.filter.fun <- function(site.filter.list, quant.vals){
  ...
  if(paste0(P1name,".",metric_var) %in% colnames(snp_df_P1)){
    ...
    } else { quantile_var <- as.numeric(quant.vals["quantile"]) }
...
quantile.matrix.function <- function(sequence.list){
  ...
  sites_to_filter_out_full_list <- lapply(SITE_tot_filters, FUN = site.filter.fun, quant.vals = sites_quant_vals)
...
target_num_snps_standard <- c(1e5) 
## INPUT THE DIFFERENT VALUES FOR THE NUMBERS OF SNPS YOU WANT TO FILTER TO ##
...
for(tt in 1:length(target_num_snps)){
  ...
  if(target_snps2 == Total_num_snps){ FINAL_filter_df_correct_size <- snp_df_no_P1_down_filters_REVERSE
    ...
    sites_to_filter_out_full_list2 <- lapply(SITE_tot_filters, FUN = site.filter.fun, quant.vals = first_closest_to_target)
    names(sites_to_filter_out_full_list2) <- SITE_tot_filters
    
    
    ## Didn't get far enough to generalize this for UG data, ie, data with different distributions between P1 & F2
   
    #if()
    P1_QUAL_filt2 <- quantile(snp_df_P1$LP.mg0P1.QUAL, first_closest_to_target$quantile, na.rm = TRUE)

    # else { P1_QUAL_filt2 <- NA }
